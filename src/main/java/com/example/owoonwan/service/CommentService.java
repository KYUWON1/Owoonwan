...
public class CommentService {
    ...
    public DeletePostCommentDto deletePostComment(Long postId, String userId,
        ...
        if(!CheckDBAndValidationForDelete(postId, commentId, userId)){
    ...
    public UpdatePostCommentDto updatePostComment(
            UpdatePostComment.Request request,
            String userId,
            Long postId
    ){
        PostComment comment =
                CheckDBAndValidationForUpdate(postId,request.getCommentId(), userId);
        comment.setContent(request.getContent());
        return UpdatePostCommentDto.fromDomain(postCommentRepository.save(comment));
    }

    private boolean CheckDBAndValidationForDelete(Long postId, Long commentId,
                                                  String userId){
    ...
    private PostComment CheckDBAndValidationForUpdate(Long postId,
                                                      Long commentId,
                                                      String userId){
        if(!postRepository.existsById(postId)){
            throw new CommentException(ErrorCode.POST_NOT_FOUND);
